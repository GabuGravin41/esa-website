...
# ============================================================================
# AUTOMATED PAYMENT FUNCTIONS - COMMENTED OUT FOR NOW
# These will be re-enabled when the automated payment system goes live
# ============================================================================
...
# ============================================================================
# END OF COMMENTED AUTOMATED PAYMENT FUNCTIONS
# ============================================================================
...
@login_required
@require_POST
def submit_manual_transaction(request):
    """Handle manual transaction code submission - simplified payment flow"""
    try:
        # Get form data
        plan_type = request.POST.get('plan_type')
        amount = request.POST.get('amount')
        ...
        # Validate the data
        if not all([plan_type, amount, transaction_code]):
            return JsonResponse({'success': False, 'error': 'Missing required fields'})
        
        # Check if transaction code already used
        if Membership.objects.filter(transaction_code=transaction_code).exists():
            return JsonResponse({'success': False, 'error': 'This transaction code has already been used'})
        
        # Create membership record with manual verification pending
        start_date = timezone.now()
        end_date = start_date + timedelta(days=365)  # 1 year membership
        ...
        membership = Membership.objects.create(
            ...
            status='pending',
            is_active=False,  # Will be activated after admin verification
            start_date=start_date,
            end_date=end_date,
            membership_number=generate_membership_number(),
        ...
        # Generate and assign membership number to user profile immediately
        try:
            profile = UserProfile.objects.get(user=request.user)
        ...
        return JsonResponse({
            'success': True,
            'message': 'Transaction submitted successfully! Admin will verify within 24 hours.',
            'membership_number': membership.membership_number,
            'redirect_url': f'/dashboard/'
        })
        
    except Exception as e:
        logging.error(f"Submit manual transaction error: {str(e)}")
        return JsonResponse({'success': False, 'error': str(e)})
